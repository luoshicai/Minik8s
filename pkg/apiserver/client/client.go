package client

import (
	"context"
	"log"
	pb "minik8s/pkg/proto"
)

// KubeletCreatePod ApiServer作为客户端给Kubelet发请求
func KubeletCreatePod(c pb.KubeletApiServerServiceClient, in *pb.ApplyPodRequest) error {
	ctx := context.Background()

	// 调用服务端 SimpleRPC 并获取响应
	reply, err := c.CreatePod(ctx, in)
	if err != nil {
		log.Fatal(err)
	}
	log.Println(reply.Status)

	return err
}

func KubeletCreateDeployment(c pb.KubeletApiServerServiceClient, in *pb.ApplyDeploymentRequest) error {
	ctx := context.Background()

	// 调用服务端 SimpleRPC 并获取响应
	reply, err := c.CreateDeployment(ctx, in)
	if err != nil {
		log.Fatal(err)
	}
	log.Println(reply.Status)

	return err
}

func KubeletDeletePod(c pb.KubeletApiServerServiceClient, in *pb.DeletePodRequest) error {
	ctx := context.Background()

	// 调用服务端 SimpleRPC 并获取响应
	reply, err := c.DeletePod(ctx, in)
	if err != nil {
		log.Fatal(err)
	}
	log.Println(reply.Status)

	return err
}
